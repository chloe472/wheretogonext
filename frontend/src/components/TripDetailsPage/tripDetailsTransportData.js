...
export const AIRPORTS_AND_CITIES = [
  ...
  { id: 'BOG', name: 'Bogota El Dorado (BOG)', city: 'Bogota', country: 'Colombia', type: 'Airport' },
...
export function searchAirportsAndCities(query, limit = 8) {
...
export function searchLocationsForSurfaceTransport(query, limit = 10) {
  const q = (query || '').trim().toLowerCase();
  if (!q) return [];
