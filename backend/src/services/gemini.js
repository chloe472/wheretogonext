...
function parsePlacesArrayFromModelText(text) {
  const raw = stripCodeFences(text);
  const jsonMatch = raw.match(/\{[\s\S]*\}/);
  if (!jsonMatch) return [];
  try {
...
/**
 * When heuristic caption parsing finds no tokens, ask Gemini for venue names (text-only).
 * Returns [] if GEMINI_API_KEY is missing or on any failure (caller treats as no-op).
 */
export async function extractPlaceNamesFromSocialCaption(linkText) {
  if (!GEMINI_API_KEY) return [];
  const caption = String(linkText || '').trim();
  if (caption.length < 3) return [];

  const userPrompt = `Extract any specific place names, restaurants, attractions, or venues mentioned in this social media caption. Return JSON only:
...
${caption.slice(0, 8000)}`;
  ...
  try {
    ...
    if (gen.error || !gen.textOut) return [];
    return parsePlacesArrayFromModelText(gen.textOut).slice(0, 8);
  } catch {
    return [];
  }
}
